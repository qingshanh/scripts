...
!(async () => {
  ...
  if ($.isNode() && msg && thetime.getDay() === 5 && thetime.getHours() > 15) {
    ...
    $.post(taskPostUrl("dwSignInfo", body), (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data) {
            if (data.code === 200) {
              console.log(`总积分${data.data.totalNum}`)
              msg += `总积分${data.data.totalNum}\n`
              console.log(`可用积分${data.data.balanceNum}`)
              msg += `可用积分${data.data.balanceNum}\n`
              console.log(`当前签到天数${data.data.signInfo.continueDay}/${data.data.signInfo.totalDay}`)
              msg += `当前签到天数${data.data.signInfo.continueDay}/${data.data.signInfo.totalDay}\n`
            } else {
              console.log("似乎签到完成了\n")
            }
          }
